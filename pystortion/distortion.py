...
class multiEpochAstrometry(object):
    ...
    def __init__(self, p=None, colNames=None):
        self.p = p
...
# import from helpers
    # def make_dir(dir):
    # def getHullPath(points):
    # histogram_with_gaussian_fit
    # f_test_probability
...
class lazAstrometryCoefficients(object):
    ...
    def displayResults(self, evalFrameNumber=1, precision=7, scaleFactorForResiduals=1., displayCorrelations=False,
                       nformat='f'):
        ...
        for i, r in enumerate(resFields):
            ...
            eval('self.{0:s}[{1:d}]'.format(r, evalFrameNumber)).display_results(precision=precision,
                                                                                 scale_factor=scaleFactor,
                                                                                 nformat=nformat)
            if displayCorrelations:
                eval('self.{0:s}[{1:d}]'.format(r, evalFrameNumber)).display_correlations()

        print('RMS  in X and Y: {0:f}  and  {1:f}'.format(self.rms[evalFrameNumber][0] * scaleFactorForResiduals,
                                                          self.rms[evalFrameNumber][1] * scaleFactorForResiduals))
        print('        chi2 in X and Y: {0:f}  and  {1:f}'.format(self.resx[evalFrameNumber].chi2,
                                                                  self.resy[evalFrameNumber].chi2))
        print('reduced chi2 in X and Y: {0:f}  and  {1:f}'.format(
            self.resx[evalFrameNumber].chi2 / self.resx[evalFrameNumber].n_freedom,
            self.resy[evalFrameNumber].chi2 / self.resy[evalFrameNumber].n_freedom))
        ...
        if (self.k >= 4) & (self.k < 10):
            ...
            self.human_readable_solution_parameters = displayRotScaleSkew(self, i=evalFrameNumber,
                                                                          scaleFactor=scaleFactor, nformat=nformat)

    def plotResiduals(self, evalFrameNumber, outDir, nameSeed, omc_scale=1., save_plot=1, omc_unit='undefined',
        ...
        omc = np.vstack((U, V)).T
        histogram_with_gaussian_fit(omc, facecolors=['b', 'r'], linecolors=['b', 'r'], labels=['X', 'Y'],
                                    xlabel='Residual O-C (%s)' % omc_unit, normed=1, save_plot=save_plot, out_dir=outDir,
                                    name_seed=nameSeed, separate_panels=True, **kwargs)
        #         histogram_with_gaussian_fit(U,facecolors=['b','r'],linecolors=['b','r'],labels=['X','Y'],xlabel='Residual O-C (%s)'%omc_unit,normed=1,save_plot=save_plot,outDir=outDir,nameSeed =nameSeed+'X')
        #         histogram_with_gaussian_fit(V,facecolors=['r','r'],linecolors=['r','r'],labels=['Y','Y'],xlabel='Residual O-C (%s)'%omc_unit,normed=1,save_plot=save_plot,outDir=outDir,nameSeed =nameSeed+'Y')
    ...
    def plotResults(self, evalFrameNumber, outDir, nameSeed, saveplot=1, xy_unit='undefined', xy_scale=1.):
    ...
    def plotRotation(self, evalFrameNumber, outDir, nameSeed, referencePointForProjection_Pix, save_plot=1,
        ...
        P = sp.symbols('p0:%d' % self.Nalm)
        Cs, polynomialTermOrder = generalBivariatePolynomial(x, y, self.k, verbose=0)
        #         polynomial = Matrix(Cs.T)*Matrix(P)
        partialMode = 4
        includeAllHigherOrders = False
        polynomial = self.getPolynomial(Cs, P, partialMode=partialMode, includeAllHigherOrders=includeAllHigherOrders)
        ...
        for j, axis in enumerate(axes):
            ...
            mode = 4
            xp, yp = self.applyPartialProjectionFunction(evalFrameNumber, mode, Cs, P, x_mesh, y_mesh,
                                                         includeAllHigherOrders=False)
    ...
    def plotLinearTerms(self, evalFrameNumber, outDir, nameSeed, referencePointForProjection_Pix, save_plot=1,
                        xy_unit='undefined', xy_scale=1.):
        ...
        P_simu = sp.symbols('p0:%d' % 3)
        Cs_simu, polynomialTermOrder_simu = generalBivariatePolynomial(x, y, 4, verbose=0)
        polynomial_simu = self.getPolynomial(Cs_simu, P_simu, partialMode=0)
    ...
    def getPartialDerivatives(self, polynomial, evalFrameNumber):
        ii = evalFrameNumber
        ...
        dfexpr_b = sp.diff(polynomial, x)
        dfexpr_c = sp.diff(polynomial, y)
        dfexpr_e = sp.diff(polynomial, x)
        dfexpr_f = sp.diff(polynomial, y)
    ...
    #
    def plotScale(self, evalFrameNumber, outDir, nameSeed, referencePointForProjection_Pix, save_plot=1,
                  xy_unit='undefined', xy_scale=1.):
        ii = evalFrameNumber
        ix = np.where(self.colNames == 'x')[0][0]
        iy = np.where(self.colNames == 'y')[0][0]
        ...
        if 1 == 1:  # plot global scale according to HST/Jay definition
            ...
            polynomial = self.getPolynomial(Cs, P, partialMode=partialMode)
            b_func, c_func, e_func, f_func = self.getPartialDerivatives(polynomial, ii)
    ...
    def getPolynomial(self, Cs, P, partialMode=0, includeAllHigherOrders=True):
        if partialMode == 0:
            # full model
            polynomial = Matrix(Cs.T) * Matrix(P)
        else:
            minMode = partialMode - 2
            maxMode = partialMode
    ...
    def applyPartialProjectionFunction(self, evalFrameNumber, partialMode, Cs, P, xx, yy, includeAllHigherOrders=False):
        ii = evalFrameNumber
        replacements_x = (','.join(['("p%d",self.Alm[%d,%d])' % (d, ii, d) for d in range(self.Nalm)]))
        replacements_y = (','.join(['("p%d",self.Alm[%d,self.Nalm+%d])' % (d, ii, d) for d in range(self.Nalm)]))
    ...
    def plotDistortion(self, evalFrameNumber, outDir, nameSeed, referencePointForProjection_Pix, save_plot=1,
        ...
        if self.k == detailed_plot_k:
            ...
            for jj, mode in enumerate(modes):
                if mode == 4:
                    ...
                    modetmp = 4
                    xp, yp = self.applyPartialProjectionFunction(evalFrameNumber, modetmp, Cs, P, x_mesh, y_mesh,
                                                                 includeAllHigherOrders=True)
                    xp_0, yp_0 = x_mesh, y_mesh
                elif mode == 6:
                    xp, yp = self.applyPartialProjectionFunction(evalFrameNumber, mode, Cs, P, x_mesh, y_mesh,
    ...
    def correctCatalogForDistortion(self, evalFrameNumber, ra, dec, referencePointForProjection_RADec, scale):
        ii = evalFrameNumber
        x_in, y_in = RADec2Pix_TAN(ra, dec, referencePointForProjection_RADec[0], referencePointForProjection_RADec[1],
                                   scale);
        P = sp.symbols('p0:%d' % self.Nalm)
        Cs, polynomialTermOrder = generalBivariatePolynomial(x, y, self.k, verbose=0)
        x_corr, y_corr = self.applyPartialProjectionFunction(evalFrameNumber, 0, Cs, P, x_in, y_in)
...
def generalBivariatePolynomial(xval, yval, k=None, degree=None, maxDegree=None, verbose=False):
    """
...
def weightedPolynomialResiduals(Alm, data):
    ...
    C, tmp = generalBivariatePolynomial(xx, yy, k, verbose=0)
    P = sp.symbols('p0:%d' % len(Alm))
...
def fitPolynomialWithUncertaintyInXandY(LHS, ximinusx0, yiminusy0, uncertainty_X, uncertainty_LHS, initialParameters,
    ...
    P = sp.symbols('p0:%d' % Nalm)
    Cs, polynomialTermOrder = generalBivariatePolynomial(x, y, k, verbose=0)
    polynomial = Matrix(Cs.T) * Matrix(P)
...
def getLazAstrometryCoefficientsFlexible(mp, k, reference_frame_number, targetIndex=None, referencePoint=None,
    ...
    Nalm = np.int(k * (k + 2) / 8);
    if Nalm > Nstars:
        print('NOT ENOUGH REFERENCE STARS')
        1 / 0
    ...
    C, polynomialTermOrder = generalBivariatePolynomial(ximinusx0, yiminusy0, k, verbose=verbose)
    for m in range(Nframes):
        ...
        #         if (masked_stars is not None) and (m != reference_frame_number):
        #             print('Masked %d stars in fit (of %d total)' % (len(masked_stars),mp.p.shape[1]))
        # #           mp.p = np.delete(mp.p,masked_stars,axis=1)
        #             weight_x[masked_stars] = 0.
        #             weight_y[masked_stars] = 0.
...
def testDistortionFit():
    ...
    lazAC.plotResiduals(evalFrameNumber, outDir, nameSeed, omc_scale=1., save_plot=saveplot, omc_unit='mas')
    lazAC.plotResults(evalFrameNumber, outDir, nameSeed, saveplot=saveplot)
...
def testDistortionFitLMC():
    ...
    k = 6
    refFrameNumber = 0
    evalFrameNumber = 1  # using linearFit only takes into account positional uncertainties in evalFrame
    #     verbose = 1
    verboseFigures = 0
    saveplot = 0
    ...
    lazAC = fitDistortion(mp, k, reference_frame_number=refFrameNumber, evaluation_frame_number=evalFrameNumber,
    ...
    lazAC.displayResults(evalFrameNumber=evalFrameNumber, scaleFactorForResiduals=1., displayCorrelations=0)
    considerUncertaintiesInXandY = 1
    lazAC_2 = fitDistortion(mp, k, reference_frame_number=refFrameNumber, evaluation_frame_number=evalFrameNumber,
                            reference_point=referencePoint, use_position_uncertainties=usePositionUncertainties,
                            use_reduced_coordinates=useReducedCoordinates,
                            consider_uncertainties_in_XY=considerUncertaintiesInXandY)
    lazAC_2.displayResults(evalFrameNumber=evalFrameNumber, scaleFactorForResiduals=1., displayCorrelations=0)
    ...
    lazAC.plotResults(evalFrameNumber, outDir, nameSeed, saveplot=saveplot, xy_scale=xy_scale, xy_unit=xy_unitStr)

    print('Simulation: Rotated by angle {0:1.1f} deg'.format(angle_deg))
    return lazAC


def construct_polynomial(Cs, P, partialMode=0, includeAllHigherOrders=True):
    if partialMode == 0:
        # full model
        polynomial = Matrix(Cs.T) * Matrix(P)
    else:
...
def display_RotScaleSkew(coefficients_x, coefficients_y, verbose=False):
    ...
    P = sp.symbols('p0:%d' % n_parameter)
    Cs, polynomialTermOrder = generalBivariatePolynomial(x, y, k)
